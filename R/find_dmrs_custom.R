...
#' from cell-type-specific DNAm data
#'
#' @description This function uses any user's specified
#' cell-type specific reference methylomes to identify
#' differentially methylated regions.
#'
#' @param mset_train A \code{RatioSet} object with normalized beta value
#' from a cell-type specific methylation experiment to be used as a
#' reference. A correct annotation must be specified.
...
find_dmrs_custom <- function(mset_train,
                           verbose=TRUE,
                           gr_target=NULL,
                           include_cpgs = FALSE,
                           include_dmrs = TRUE,
  ...
  if(!is.null(gr_target)){
    ...
    zz <- findOverlaps(granges(mset_train), gr_target)
    mset_train <- mset_train[queryHits(zz), ]
    if(verbose){
  ...
  p_beta <- data.matrix( getBeta(mset_train) ) # beta values as numeric matrix
